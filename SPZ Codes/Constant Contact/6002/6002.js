...
hiddenValue('#6002 | Constant Contact | LPs | Form In Modal', 'SPZ_6002_V1')
...
if (window.location.pathname.includes('/landing2/ai-marketing')) {
  template_formUniqueSelector = "#meta-sign-up-form-em-pwd-fname-tel-org-req-gsff";
} else {
...
function addForm() {
  ...
  document.body.classList.add('spz_6002_v1');
  const formTemplate = `
  ...
  if (!document.querySelector('.spz_modal_wrap')) {
    ...
    if (window.location.pathname.includes('/buynow/')) {
      ...
      template_inputsSelectors = ["#email", "#new-password", "#given-name", "#organization"];
      template_labelValues = ["Email", "Password", "First Name", "Organization"];
