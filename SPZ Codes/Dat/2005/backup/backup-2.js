...
function handleInputEvents(inputsSelectors, fieldsToHide) {
	document.querySelectorAll(inputsSelectors.join(',')).forEach(input => {
			['keyup', 'input'].forEach(event => {
					input.addEventListener(event, function () {
							validateAndUnhideFields(fieldsToHide);
					});
			});
	});
}

// Function to validate required fields and unhide specified fields
function validateAndUnhideFields(fieldsToHide) {
	const firstName = document.querySelector('[name="firstname"], [name="name"]')?.value.trim();
	const lastName = document.querySelector('[name="lastname"], [name="LastName"]')?.value.trim();
	const email = document.querySelector('[name="email"], [name="Email"]')?.value.trim();
	const company = document.querySelector('[name="company"]')?.value.trim();
	const jobTitle = document.querySelector('[name="jobtitle"]')?.value.trim() || true;
	const phone = document.querySelector('[name="phone"], [name="Phone"]')?.value.trim() || true;

	// Validate email format
	const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/;
	const emailValid = emailPattern.test(email);

	if (firstName && lastName && emailValid && company && jobTitle && phone) {
			fieldsToHide.forEach(selector => {
					const field = document.querySelector(selector);
					if (field) {
							const wrapper = field.closest('.spz-input-wrap');
	...
	document.querySelector('[name="email"], [name="Email"]').addEventListener('blur', function () {
...
function showError(message) {
	...
	const svgIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none"><g clip-path="url(#clip0_9232_488)"><path d="M7.99967 5.33334V8.00001M7.99967 10.6667H8.00634M14.6663 8.00001C14.6663 11.6819 11.6816 14.6667 7.99967 14.6667C4.31778 14.6667 1.33301 11.6819 1.33301 8.00001C1.33301 4.31811 4.31778 1.33334 7.99967 1.33334C11.6816 1.33334 14.6663 4.31811 14.6663 8.00001Z" stroke="#FF0F00" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></g><defs><clipPath id="clip0_9232_488"><rect width="16" height="16" fill="white"></rect></clipPath></defs></svg>`;
	...
	if (!errorMsg) {
			...
			document.querySelector('[name="email"], [name="Email"]').after(errorMsg);
	}
}

// Function to remove error
function removeError() {
	const email = document.querySelector('[name="email"], [name="Email"]').value;
	const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/;
...
if (window.location.pathname === '/sales-inquiry-professional-services' || window.location.pathname === '/sales-inquiry-promotion-2' || window.location.pathname === '/sales-inquiry-rateview') {
	...
	handleInputEvents(inputsSelectors, fieldsToHide);
}
