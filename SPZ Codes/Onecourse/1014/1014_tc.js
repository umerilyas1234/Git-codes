...
function run1014TestTC() {
  ...
  function resizeChanges() {
    if (window.location.href.split("?")[0].split("#")[0] === 'https://www.awrusa.com/' || window.location.href.split("?")[0].split("#")[0] === 'https://oncourse-portal-qa.ohscloud.net/' || window.location.search.includes('_vis_preview_data')) {
      if (window.innerWidth >= 1024) {
        ...
        if (!document.querySelector('.spz-banner-tags')) {
          if (window.location.href.split("?")[0].split("#")[0] === "https://www.awrusa.com/" || window.location.href.split("?")[0].split("#")[0] === "https://oncourse-portal-qa.ohscloud.net/" || window.location.href.includes("?_vis_preview_data")) {
            const heading = document.querySelector('.awr-rich-text h2');
            if (heading) {
  ...
  function spz5002Test() {
    if (!document.querySelector('body').classList.contains('spz_5002')) {
      ...
      function appendUniqueQueryParam(key, value) {
        const url = new URL(window.location.href);
        const params = url.searchParams;

        // Check if SAME key/value pair already exists
        const exists = [...params.getAll(key)].includes(value);
  ...
  function removeTest() {
    ...
    if (document.querySelector('form.awr-find-your-protection-plan__wrapper input#zip-code')) {
      ...
      if (document.querySelector('form.awr-find-your-protection-plan__wrapper button span p')) {
  ...
  function removeTest5002() {
    if (document.querySelector('body.spz_5002')) {
  ...
  function observerForLoadingBlock() {
    ...
    const callback = function (mutationsList, observer) {
      ...
      var isInList = ["/plans/all-plans", "/plans/water-sewer", "/plans/heating-cooling", "/plans/electrical-gas", "/plans/appliances", "/plans/whole-home"].some(function (path) { return currentPage.includes(path); });
      ...
      if (pageList.includes(currentPage)) {
        if (!document.querySelector('.spz1008_testimonialSlider_v1')) {
          if (document.querySelector('.spz_5002')) {
            removeTest5002();
          }
          running = true;
      ...
      } else if (isInList && !isHomepage && !isZipExcludedFilled()) {
        if (!document.querySelector('.spz_5002')) {
          if (document.querySelector('.spz_1007_v1')) {
            removeTest();
          }
          console.log("URL in list and not homepage, running spz5002Test()");
          running = true;
          isProcessing = true; // Set processing flag
          testRemoved = false;
          spz5002Test();
