...
<div class="spz_sections">
  ...
  <div class="spz_testimonials">
    ...
    const testName = "homepage_variant";
    vwoClient.setCustomDimension(testName, "homepage_variant"); 
...
var pageList = [
  ...
  "https://www.awrusa.com/cart",
  "https://www.awrusa.com/cart/payment"
];
...
function runCodeBasedOnUrl() {
  ...
  if (currentPage === "https://www.awrusa.com/") {
  ...
  } else if (pageList.includes(currentPage)) {
    if (running) return; // Prevent multiple executions
    running = true;
...
// Also run on window load (in case of page refresh)
...
const observer = new MutationObserver(() => {
  ...
  if (currentPage === previousUrl) {
    ...
    var isHomepage = currentPage === "https://www.awrusa.com/";
    var isInList = pageList.includes(currentPage);
    ...
    if (selectorElement) {
      ...
      if (isHomepage && (!hasSpzClass || !hasSpzSections)) {
        if (previousUrl && pageList.includes(previousUrl)) {
          // removeTest();
        }
        runCodeBasedOnUrl();
