function runCode() {
  if (!document.body.classList.contains('spz-6009')) {
    ...
    const checkForm = setInterval(() => {
      ...
      const formElement = document.querySelectorAll(`#${formId}`);

      if (formElement.length > 0) {
        console.log('interval found');
        clearInterval(checkForm);
        console.log("form length 111111 = " + formElement.length);
        setTimeout(() => {
        ...
        console.log("form length =", document.querySelector(`#${formId}`).querySelectorAll('form'));
