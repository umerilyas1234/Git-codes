...
function injectVariant6015() {
	...
	if (infoSection) {
			...
			if (capabilitiesHeaderSection) {
					...
					if (capabilitiesSection) {
							...
							// Track which accordion's image is shown; only skip update when user clicks that same tab again (closing it)
							let currentAccordionImageControl = 'accordion-5-0';
							...
							capabilitiesSection.querySelectorAll('button.c--accordion-a__hd').forEach(button => {
									button.addEventListener('click', (e) => {
											...
											// Don't update image when user clicks the tab we're already showing (they're closing the active accordion)
											if (dataAccordianControl === currentAccordionImageControl) {
													return;
											}
											currentAccordionImageControl = dataAccordianControl;
	...
	const variantName = `Variation2_` + expName; //variantName should be variant_, true_control_ etc.
