if (window.location.href.includes("/product/credit") || window.location.href.includes("/product/spend-and-expense") || window.location.href.includes("product/virtual-cards")) {
	function loadTestCode7001() {
		if (!document.querySelector('body').classList.contains('spz_t7001')) {
			...
			if (document.querySelector( getLabelSelector('numberOfEmployees'))) {
				document.querySelector( getLabelSelector('numberOfEmployees')).innerHTML = document.querySelector(getLabelSelector('numberOfEmployees')).innerHTML.replace('Number Of', 'Number of');
			}
			...
			//submit button 
			if (document.querySelector(getFieldSelector('se-form-button'))) {
				document.querySelector(getFieldSelector('se-form-button')).classList.add('spz-submitcta-1002', 'spz_7001_v1');
				document.querySelector(getFieldSelector('se-form-button')).value = 'Submit';
			}
			...
			if (window.location.href.includes("/product/credit")) {
				...
				document.querySelectorAll('a[href="#formjump"]').forEach(function (el) {
					el.href = '#formjumplink';
				})
			}
...
if (window.location.href.includes("/product/accounts-payable")) {
	function loadTestCode7001() {
		if (!document.querySelector('body').classList.contains('spz_t7001')) {
			...
			document.querySelectorAll('.signup-direct input[placeholder], .signup-direct select, #signupType.signupType').forEach(function (inputEl) {
				var wrapper = document.createElement('div');
				wrapper.className = 'form-field-wrap';
				inputEl.parentNode.insertBefore(wrapper, inputEl);
				wrapper.appendChild(inputEl);
			...
			if (document.querySelector('.signup-direct form.form-wrapper #numberOfEmployees + label')) {
				document.querySelector('.signup-direct form.form-wrapper #numberOfEmployees + label').innerHTML = document.querySelector('#numberOfEmployees + label').innerHTML.replace('Number Of', 'Number of');
			}
			if (document.querySelector('.signup-direct form.form-wrapper #signupType + label')) {
				document.querySelector('.signup-direct form.form-wrapper #signupType + label').innerHTML = "Business Type";
			}
			if (document.querySelector('.signup-direct form.form-wrapper #accountingSoftware + label')) {
				document.querySelector('.signup-direct form.form-wrapper #accountingSoftware + label').innerHTML = "Which accounting software do you use?";
			}
			if (document.querySelector('.signup-direct form.form-wrapper #msDynamicsOption + label')) {
				document.querySelector('.signup-direct form.form-wrapper #msDynamicsOption + label').innerHTML = "Select your MS Dynamics solution:";
			}

			//input even listerer
			const allInput = document.querySelectorAll('.signup-direct form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]), .signup-direct form select');
			function labelChange(e) {
				if (!e) return;

				// Determine the appropriate parent class
				let formFieldWrap = e.closest('.form-field-wrap') || e.closest('.form-options') || e.closest('.form-fields-hide ') || e.closest('.form-dropdown');
			...
			document.querySelector('.signup-direct #numberOfEmployees').addEventListener('input', function (e) {
				// Replace any non-digit character with an empty string
				this.value = this.value.replace(/\D/g, '');
			});
			...
			setTimeout(() => {
				// Show form fields if pre-filled
				const firstName = document.querySelector('.signup-direct #firstName').value;
				const lastName = document.querySelector('.signup-direct #lastName').value;
				const email = document.querySelector('.signup-direct #email').value;
				const name = document.querySelector('.signup-direct #name').value;
				...
				if (firstName !== '' && lastName !== '' && email !== '' && name !== '' && emailValid) {
					// Select the elements to unhide
					const fieldsToShow = ['.signup-direct #phone', '.signup-direct #accountingSoftware', '.signup-direct #numberOfEmployees', '.signup-direct #signupType'];
			...
			// change position
			document.querySelector('.signup-direct #lastName').closest('.form-field-wrap').insertAdjacentElement('afterend', document.querySelector('.signup-direct #email').closest('.form-field-wrap'));
			document.querySelector('.signup-direct #email').closest('.form-field-wrap').insertAdjacentElement('afterend', document.querySelector('.signup-direct #name').closest('.form-field-wrap'));
			document.querySelector('.signup-direct #phone').closest('.form-field-wrap').insertAdjacentElement('afterend', document.querySelector('.signup-direct #numberOfEmployees').closest('.form-field-wrap'));
			// document.querySelector('.signup-direct #numberOfEmployees').closest('.form-field-wrap').insertAdjacentElement('afterend', document.querySelector('.signup-direct select#signupType').closest('.form-field-wrap'));
			// document.querySelector('.signup-direct select#signupType').closest('.form-field-wrap').insertAdjacentElement('afterend', document.querySelector('.signup-direct #accountingSoftware').closest('.form-field-wrap'));
			document.querySelector('.signup-direct #accountingSoftwareContainer').insertAdjacentElement('afterend', document.querySelector('.signup-direct #msDynamicsOption').closest('.form-field-wrap'));
			...
			//submit button 
			if (document.querySelector('.signup-direct #se-form-button')) {
				document.querySelector('.signup-direct #se-form-button').classList.add('spz-submitcta-1002', 'spz_7001_v1');
				document.querySelector('.signup-direct #se-form-button').value = 'Submit';
			}
			if (document.querySelector('.signup-direct #directSignup .button-signup-2')) {
				document.querySelector('.signup-direct #directSignup .button-signup-2').classList.add('spz-submitcta-1002', 'spz_7001_v1');
				document.querySelector('.signup-direct #directSignup .button-signup-2').value = 'Submit';
			}

			//disclaimer 
			document.querySelector('.signup-direct .button-disclaimer').innerHTML = ` By continuing, you agree to BILL's <a href="/legal/terms-of-service" target="_blank" class="text-size-small no-wrap">Terms of Service</a> and&nbsp;<a href="/privacy" target="_blank" class="text-size-small no-wrap">Privacy Notice</a>.`
			...
			// const msOptions = document.querySelector('.spz_t7001 .signup-direct form.form-wrapper #accountingSoftwareContainer .form-field-wrap:last-child');
			// const accountingContainer = document.querySelector('.spz_t7001 .signup-direct form.form-wrapper #accountingSoftwareContainer');
			// if(accountingContainer){
			// 	accountingContainer.insertAdjacentElement('afterend', msOptions);
			// }

			document.querySelector('.signup-direct select#msDynamicsOption').parentElement.classList.add('msDynamicsOption_wrapper');

			// Hide fields on page load
			const fieldsToHide = ['.signup-direct #phone', '.signup-direct #numberOfEmployees', '.signup-direct #signupType', '.signup-direct #accountingSoftware'];
			...
			document.querySelectorAll('.spz_t7001 .signup-direct form.form-wrapper .form-field-wrap input').forEach(input => {
				input.addEventListener('keyup', function () {
					const firstName = document.querySelector('.signup-direct #firstName').value;
					const lastName = document.querySelector('.signup-direct #lastName').value;
					const email = document.querySelector('.signup-direct #email').value;
			...
			document.querySelector('.signup-direct #email').addEventListener('blur', function () {
				const email = this.value;
				const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/;
				const emailValid = emailPattern.test(email);
