function loadTestCode5003() {
  if (!document.querySelector("body").classList.contains("spz_5003_v1")) {
    document.querySelector("body").classList.add("spz_5003_v1");
    ...
    const formUniqueSelector = ".form-wrapper" //<--- form uqniue selector goes here. e.g.
    ...
    const formLoaded = setInterval(() => {
      if (document.querySelector(formUniqueSelector) && document.querySelectorAll(`${formUniqueSelector} input`).length > 0) {
        ...
        //form modification
        //adding wrapper
        for (let i = 0; i < document.querySelectorAll('.spz_5003_v1 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]),.spz_5003_v1 form select').length; i++) {
          let target = document.querySelectorAll('.spz_5003_v1 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]),.spz_5003_v1 form select')[i];
          let wrapper = document.createElement('div');
          wrapper.classList.add('spz-input-wrapper', document.querySelectorAll('.spz_5003_v1 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]),.spz_5003_v1 form select')[i].getAttribute("id"));
          target.parentNode.insertBefore(wrapper, target);
          wrapper.appendChild(target);
        }
        //adding label
        for (let i = 0; i < document.querySelectorAll('.spz_5003_v1 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]),.spz_5003_v1 form select').length; i++) {
          let label = document.querySelectorAll('.spz_5003_v1 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]),.spz_5003_v1 form select')[i].getAttribute("placeholder") == "Number Of Employees" ? "Number of Employees" : document.querySelectorAll('.spz_5003_v1 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]),.spz_5003_v1 form select')[i].getAttribute("placeholder")
          if (!label && document.querySelectorAll('.spz_5003_v1 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]),.spz_5003_v1 form select')[i].querySelector('option')) {
            label = document.querySelectorAll('.spz_5003_v1 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]),.spz_5003_v1 form select')[i].querySelector('option').textContent;
          }
          document.querySelectorAll('.spz_5003_v1 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]),.spz_5003_v1 form select')[i].insertAdjacentHTML("afterend", `
            <label for="${document.querySelectorAll('.spz_5003_v1 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]),.spz_5003_v1 form select')[i].getAttribute("id")}">${label}</label> 
         `)
        }
        // change position
        document.querySelector('.spz_5003_v1 .spz-input-wrapper.email').insertAdjacentElement('afterend', document.querySelector('.spz_5003_v1 .spz-input-wrapper.name'));
        document.querySelector('.spz_5003_v1 .spz-input-wrapper.phone').insertAdjacentElement('afterend', document.querySelector('.spz_5003_v1 .spz-input-wrapper.numberOfEmployees'));

        //products style
        document.querySelector('.spz_5003_v1 #form-panel-3 .text-size-tiny').textContent = "(Check all that apply)"

        //select arrow
        for (let x = 0; x < document.querySelectorAll('.spz_5003_v1 form select').length; x++) {
          document.querySelectorAll('.spz_5003_v1 form select')[x].parentNode.insertAdjacentHTML("beforeend", `
            <div class="arrow"></div>  
            `)
        }
        //submit button 
        document.querySelector('.spz_5003_v1 .submit-button.button-signup-2').classList.add("spz-submitcta-1004")

        //input even listerer
        const allInput = document.querySelectorAll('.spz_5003_v1 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]),.spz_5003_v1 form select');
        ...
        for (let x = 0; x < document.querySelectorAll('.spz_5003_v1 #accountingSoftwareContainer select').length; x++) {
          document.querySelectorAll('.spz_5003_v1 #accountingSoftwareContainer select')[x].querySelector('option').disabled = true;
        }
        ...
        const checkSelectLabel = () => {
          ...
          document.querySelector('.spz_5003_v1 .spz-input-wrapper.accountingSoftware label').textContent = labelText;
          document.querySelector('.spz_5003_v1 .spz-input-wrapper.accountingSoftware select option').textContent = labelText;
        }
        checkSelectLabel();
        ...
        const NOEInterval = setInterval(function () {
          if (document.querySelector('.spz_5003_v1 .spz-form-wrap .spz-input-wrapper.numberOfEmployees input')) {
            clearInterval(NOEInterval)
            document.querySelector('.spz_5003_v1 .spz-form-wrap .spz-input-wrapper.numberOfEmployees input').addEventListener('keypress', function (event) {
        ...
        document.querySelector('.spz_5003_v1').classList.add(browser);
        triageCode();
      }
    })
