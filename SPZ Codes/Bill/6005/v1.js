const bodyInterval6002 = setInterval(function () {
  ...
  document.querySelector('body').classList.add("spz_stickyCTA")

  localStorage.setItem('bdcAbTest12', '6005-v1');
  ...
  const formLoaded = setInterval(() => {
      ...
      const signupOptions = document.querySelectorAll('.spz_6002 #signupType .form-option-field span');
      for (let i = 0; i < signupOptions.length; i++) {
          const optionText = signupOptions[i].textContent;
          document.querySelector('.spz_6002 #businessType').insertAdjacentHTML("beforeend", `
          <option value="${optionText}">${optionText}</option>
          `);
      }
      
      // Add change event listener
      document.querySelector('.spz_6002 #businessType').addEventListener("change", function () {
          const selectedIndex = this.selectedIndex - 1; 
          if (selectedIndex >= 0) { // Check if a valid option is selected
          document.querySelectorAll('.spz_6002 #signupType .form-option-field')[selectedIndex].querySelector('input').click();
          }
      });
      //disclaimer 
  ...
  ************************/
  ...
  function addForm(formData, formSelector) {
      ...
      const stickyFooterContent = {
          sectionLeftHTML: `
              <div class="spz-rating-image">
                  <img src="//res.cloudinary.com/spiralyze/image/upload/v1738308449/bill/6005/G2-logo.svg" alt="G2 logo" class="spz-G2logo">
                  <img src="//res.cloudinary.com/spiralyze/image/upload/v1738308450/bill/6005/ratingStars.svg" alt="Ratings" class="spz-Stars-logo">
              </div>
              <div class="spz-ratings">
                  <p><strong>4.4</strong> (2,800+ reviews)</p>
                  <img src="//res.cloudinary.com/spiralyze/image/upload/v1738308450/bill/6005/ratingStars.svg" alt="Stars logo" class="spz-Stars-logo">
              </div>`,
          sectionMiddleHTML: `Auto-pay vendors. Track payments. Two-way sync with your accounting software.`,
          sectionRightHTML: `<a href="javascript:;" class="button w-button spz_6005_v1" id="stickyButton">Get a Demo</a>`,
      };
      ...
      ************************/
      ...
      // Hide CTA bar on page load if previously closed
      if (localStorage.getItem("footerHidden") === "true") {
          document.querySelector(".spz-sticky-footer")?.remove();
          document.querySelector('.spz_6002')?.classList.remove('spz_stickyCTA');
      }
      ...
      document.addEventListener("click", function (e) {
          if (e.target.id == "stickyButton") {
              document.querySelector('.spz_6002 .new-hero-section .left .form .custom_cta').click();
          }
      });

      document.addEventListener("click", function (e) {
          if (e.target.classList.contains("sticky_close")) {
              document.querySelector('.spz_6002')?.classList.remove('spz_stickyCTA');
          }
      });
      // Sticky Code End
