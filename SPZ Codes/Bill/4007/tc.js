const bodyInterval4002 = setInterval(function () {
	if (document.querySelector('body') && !document.querySelector('.spz-4002')) {
		...
		localStorage.setItem('bdcAbTest11', '4007-con');
  ...
  `)
		...
		const formLoaded = setInterval(() => {
			if (document.querySelector(formUniqueSelector) && document.querySelectorAll(`${formUniqueSelector} input`).length > 0) {
				...
				//form modification
				//adding wrapper
				for (let i = 0; i < document.querySelectorAll('.spz-4002 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="hidden"]),.spz-4002 form select').length; i++) {
					let target = document.querySelectorAll('.spz-4002 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="hidden"]),.spz-4002 form select')[i];
					let wrapper = document.createElement('div');
					wrapper.classList.add('spz-input-wrapper', document.querySelectorAll('.spz-4002 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="hidden"]),.spz-4002 form select')[i].getAttribute("id"));
					target.parentNode.insertBefore(wrapper, target);
					wrapper.appendChild(target);
				}
				//adding label
				for (let i = 0; i < document.querySelectorAll('.spz-4002 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="hidden"]),.spz-4002 form select').length; i++) {
					let label = document.querySelectorAll('.spz-4002 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="hidden"]),.spz-4002 form select')[i].getAttribute("placeholder") == "Number Of Employees" ? "Number of Employees" : document.querySelectorAll('.spz-4002 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="hidden"]),.spz-4002 form select')[i].getAttribute("placeholder")
					if (!label && document.querySelectorAll('.spz-4002 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="hidden"]),.spz-4002 form select')[i].querySelector('option')) {
						label = document.querySelectorAll('.spz-4002 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="hidden"]),.spz-4002 form select')[i].querySelector('option').textContent;
					}
					document.querySelectorAll('.spz-4002 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="hidden"]),.spz-4002 form select')[i].insertAdjacentHTML("afterend", `
          <label for="${document.querySelectorAll('.spz-4002 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="hidden"]),.spz-4002 form select')[i].getAttribute("id")}">${label}</label> 
      `)
				}
				// change position
				document.querySelector('.spz-4002 .spz-input-wrapper.firstName').insertAdjacentElement('beforebegin', document.querySelector('.spz-4002 .spz-input-wrapper.email'))
				document.querySelector('.spz-4002 .spz-input-wrapper.numberOfEmployees').insertAdjacentElement('afterend', document.querySelector('#form-panel-3 .signupType'));
				if (document.querySelector('.spz-4002 .g-recaptcha')) {
					document.querySelector('.spz-4002 .popup-wrapper ').insertAdjacentElement("beforeend", document.querySelector('.spz-4002 .g-recaptcha'))
				}

				//select condition checking
				// const checkSelectCondition = () => {
				// 	for (let i = 0; i < document.querySelectorAll('.spz-4002 .spz-input-wrapper').length; i++) {
				// 		if (document.querySelectorAll('.spz-4002 .spz-input-wrapper')[i].querySelector('.hide')) {
				// 			document.querySelectorAll('.spz-4002 .spz-input-wrapper')[i].classList.add('hide')
				// 		} else {
				// 			document.querySelectorAll('.spz-4002 .spz-input-wrapper')[i].classList.remove('hide')
				// 		}
				...
				//select arrow
				for (let x = 0; x < document.querySelectorAll('.spz-4002 form select').length; x++) {
					document.querySelectorAll('.spz-4002 form select')[x].parentNode.insertAdjacentHTML("beforeend", `
          <div class="arrow"></div>  
          `)
				}
				//submit button 
				document.querySelector('.spz-4002 .submit-button.button-signup-2').value = "Instant Access";
				document.querySelector('.spz-4002 .submit-button.button-signup-2').classList.add("spz-submitcta-4002")
				//disclaimer 
				document.querySelector('.spz-4002 .button-disclaimer').innerHTML = `
          ...
          `
				//input even listerer
				const allInput = document.querySelectorAll('.spz-4002 form input:not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="hidden"]),.spz-4002 form select');
				...
				for (let x = 0; x < document.querySelectorAll('.spz-4002 #accountingSoftwareContainer select').length; x++) {
					document.querySelectorAll('.spz-4002 #accountingSoftwareContainer select')[x].querySelector('option').disabled = true;
				}
				//browser detection
				...
				document.querySelector('.spz-4002').classList.add(browser);

				document.querySelector('.spz-4002 .spz-form-wrap .close-button').addEventListener("click", function () {
