  ...
  const bodyInterval3001 = setInterval(function () {
    if (document.body &&
      ...
      if (window.location.pathname.includes('/sba-loans') || window.location.pathname.includes('/partners-sf') || window.location.pathname.includes('/small-business-financing') || window.location.pathname.includes('/partners-lf')) {
        sectionSelector = document.querySelector('.spz_3004_v2 .main-content-div');
        formSelector = document.querySelector('.spz_3004_v2 .main-content-div .form-div');
        ...
        sectionSelector = document.querySelector('.spz_3004_v2 .lp-header-with-cta');
        formSelector = document.querySelector('.spz_3004_v2 .lp-pwf-cont .application-form-wrap-lp-spec');
      } else if (window.location.pathname.includes('/simple-a')) {
        document.querySelector('body').classList.add("spz_3001_simple_a");
        sectionSelector = document.querySelector('.spz_3004_v2 .main-content-div');
        formSelector = document.querySelector('.spz_3004_v2 .main-content-div .form-div');
      }
      let headerLogoHref = 'https://www.lendio.com/';
      let headerLogoHrefTarget = '';
      if (window.location.pathname.includes('/lp/direct/search-small-business-loans')) {
        const controlTag = document.querySelector('.spz_3004_v2 .navbar-container .nav-landing-logos-container a');
        if (controlTag) {
          headerLogoHref = controlTag.href;
        }
      } else if (window.location.pathname.includes('/simple-a')) {
        headerLogoHrefTarget = 'target="_blank"';
        const controlTag = document.querySelector('.spz_3001_v1 .dynamic-lp-inner div:first-child div:first-child a');
        if (controlTag) {
          headerLogoHref = controlTag.href;
        }
      } else if (window.location.pathname.includes('/partners-sf') || window.location.pathname.includes('/partners-lf')) {
        headerLogoHrefTarget = 'target="_blank"';
      }
      ...
      function newSection() {
          ...
          <div class="form_section">
            <div class="auto_container">
              <div class="form_inner">
                <div class="main_logo">
                  <a href="${headerLogoHref}" ${headerLogoHrefTarget}><img src="//res.cloudinary.com/spiralyze/image/upload/f_auto/lendio/3004/logo.webp"
                      alt="Lendio logo"></a>
                </div>
                <div class="hero_main">
                  <div class="hero_left">
                    <div class="form_heading">
                      ...
                      <ul>
                        <li>
                          <p><strong>Fast application.</strong> Apply for funding in minutes. No effect on your credit. Get funds as soon as next day.   </p>
                        </li>
                        <li>
                          <p><strong>75+ Lenders.</strong> See offers from over 75 vetted lenders, including SBA-approved lenders. Compare interest rates, APR, terms, and more.</p>
                        </li>
                        <li>
        ...
        if (window.location.pathname.includes('/sba-loans')) {
          document.querySelector('.form_heading h3').innerHTML = `SMALL BUSINESS ADMINISTRATION (SBA) LOANS`;
          document.querySelector('.form_heading h2').innerHTML = `Get an SBA loan in as little as <strong>14 days.</strong> <sup>1</sup>`;
          document.querySelector('.form_heading ul').innerHTML = `
            <li>
              <p><strong>Fast application. </strong> Apply in minutes. No effect on your credit. Up to $5 million. Terms up to 30 years.</p>
            </li>
            <li>
              <p><strong>SBA Lenders. </strong> See offers from multiple SBA-approved lenders. Compare APR, terms, etc.</p>
            </li>
            <li>
              <p><strong>Largest provider. </strong> Over 12,200 SBA loans awarded, totaling more than $475 million.</p>
            </li>
          `;
          setTimeout(() => {
            if(document.querySelector('.site-footer-disclaimer')){
              document.querySelector('.site-footer-disclaimer').innerHTML = `<p>California loans made pursuant to the California
              Financing Law, Division 9 (commencing with
              Section 22000) of the Finance Code. All such
              loans made through Lendio Partners, LLC, a wholly-
              owned subsidiary of Lendio, Inc. and a licensed
              finance lender/broker, California Financing Law
              License No. 60DBO-44694.</p>
                        <p>1. Time to fund depends upon the product that you 
              select, and can be as little as 14 days, but may be longer.</p>
              <p>2. Filling out an application for business funding
              and submitting it to our funding partners will not
              impact your personal credit score. However,
              depending on the product and lender, accepting a
              funding offer may result in a hard credit inquiry,
              which could affect your personal credit score.</p>
              `;
      ...
      let LAST_ACTIVE_CAROUSEL_ITEM = null;
      let FIRST_TIME_FOCUS = true;

      function focusHandler(e, shadowRoot) {
        setTimeout(() => {
          if (LAST_ACTIVE_CAROUSEL_ITEM !== shadowRoot.querySelector('#pwf-1 .carousel-inner .carousel-item.active')) {
            const startTime = Date.now();
            const interval = setInterval(() => {
              const activeCarouselItem = shadowRoot.querySelector('#pwf-1 .carousel-inner .carousel-item.active');
              if (activeCarouselItem) {
                const inputs = activeCarouselItem.querySelectorAll('input');
                if (inputs.length > 0) {
                  inputs[0].focus();
                  clearInterval(interval);
                  LAST_ACTIVE_CAROUSEL_ITEM = shadowRoot.querySelector('#pwf-1 .carousel-inner .carousel-item.active');
                  return;
                }
              }
      ...
      function keydownHandler(e, shadowRoot) {
        if (e.key === 'Enter') {
          focusHandler(e, shadowRoot);
        }
      }

      function backLinkHandler(e, shadowRoot) {
        LAST_ACTIVE_CAROUSEL_ITEM = shadowRoot.querySelector('#pwf-1 .carousel-inner .carousel-item.active');
      }
        ...
        formInterval = setInterval(function () {
          if (document.querySelector('lendio-start')) {
            if (document.querySelector('lendio-start').shadowRoot) {
              ...
              // Function to ensure sup element always has value '2'
              function ensureSupValueIsTwo() {
                const supElement = shadowRoot.querySelector('div:has(.disclosure-tooltip) small sup');
                if (supElement && supElement.innerHTML !== '2') {
                  supElement.innerHTML = '2';
                }
              }
              
              // Set initial value
              if(shadowRoot.querySelector('div:has(.disclosure-tooltip) small sup')){
                ensureSupValueIsTwo();
                
                // Set up MutationObserver to watch for changes
                const supElement = shadowRoot.querySelector('div:has(.disclosure-tooltip) small sup');
                if (supElement) {
                  const supObserver = new MutationObserver((mutations) => {
                  ...
                  supObserver.observe(supElement, {
                    childList: true,
                    characterData: true,
                    subtree: true
                  });
                  
                  // Also watch the parent element in case the sup is recreated
                  const parentElement = shadowRoot.querySelector('div:has(.disclosure-tooltip) small');
                  if (parentElement) {
              ...
              const disclosure = shadowRoot.querySelector('#pwf-1 div:has(.disclosure-tooltip) small');
              if (disclosure) {
                disclosure.innerHTML = disclosure.innerHTML.replace(/(<sup>.*?<\/sup>)(.*?)(\.)/i, '$2$3$1');
              }
              const observer = new MutationObserver((mutationsList) => {
                ...
                shadowRoot.querySelectorAll('#pwf-1 .carousel-inner .carousel-item.active input').forEach(item => {
                  item.removeEventListener('keydown', (e) => keydownHandler(e, shadowRoot));
                  item.addEventListener('keydown', (e) => keydownHandler(e, shadowRoot));
                });

                const firstBtn = shadowRoot.querySelector('.btn-action');
                if (firstBtn) {
                  shadowRoot.querySelectorAll('.btn-action').forEach(element => {
                    if (!element.classList.contains('spz3004_v2')) element.classList.add('spz3004_v2');
                    element.removeEventListener('click', (e) => focusHandler(e, shadowRoot));
                    element.addEventListener('click', (e) => focusHandler(e, shadowRoot));
                  });
                }

                const backLink = shadowRoot.querySelector('#pwf-1 .back-link');
                if (backLink) {
                  backLink.removeEventListener('click', (e) => backLinkHandler(e, shadowRoot));
                  backLink.addEventListener('click', (e) => backLinkHandler(e, shadowRoot));
                }
