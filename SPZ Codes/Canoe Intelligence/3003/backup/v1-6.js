function spz3003() {
  if (!document.querySelector('body').classList.contains('spz_3003') && document.querySelector('#sg-popup-content-wrapper-6993 .sgpb-main-html-content-wrapper')) {
    ...
    const formConfigs = {
      'solutions/canoe-intelligence/': { id: '6998', delay: 1500 },
      'solutions/canoe-connect/': { id: '7500', delay: 1500 },
      'solutions/canoe-asset-data/': { id: '6959', delay: 1500 }
    };
    ...
    setTimeout(function () {
      ...
      setTimeout(() => {
        ...
        console.log('Form moved to demo_formOuter');
    ...
    if (location.origin === 'https://canoeintelligence.com' &&
      ...
      document.querySelector('.heroContent').insertAdjacentHTML('afterbegin', `<div class="heroLeft">
          ...
          <h1>Spend 75% less time on <br> alts <br> administration.</h1>
          <ul>
              <li><strong>Document management.</strong> Automatically retrieve post-investment documents from email inboxes & 3000+ GP portals. Automatically categorize & store in one place. </li>
              <li><strong>Data extraction.</strong> Instantly extract & normalize key data points with a 99% accuracy rate. Expedite workflows across capital calls, distributions, fund metrics, and more with AI.</li>
              <li><strong>Deeper intelligence.</strong> Drill down into portfolio companies across funds with a 5-day SLA so you can respond to market changes in real time. </li>
          </ul>
      </div>
      <div class="heroRight">
        <div class="load-wrapper"><div class="lds-dual-ring"></div></div>
        <div class="tab_form demo_formOuter">
        </div>
          <ul>
            <li><strong>Document management.</strong> Automatically retrieve post-investment documents from email inboxes & 3000+ GP portals. Automatically categorize & store in one place. </li>
            <li><strong>Data extraction.</strong> Instantly extract & normalize key data points with a 99% accuracy rate. Expedite workflows across capital calls, distributions, fund metrics, and more with AI.</li>
      ...
      `);
      document.querySelector('.heroContent + .container h3').textContent = "Trusted by over 425 clients, from single family offices to large institutions."
    } else if (location.href.includes('solutions/canoe-intelligence/')) {
      document.body.classList.add('canoe_intelligence');
      setTimeout(() => {
        document.querySelector('.sgpb-content-6998 .form_wrapper .sgpb-main-html-content-wrapper').insertAdjacentHTML('beforeend', `<a class="btn btn btn-fill access_brochure" href="/wp-content/uploads/2024/07/Canoe-Intelligence-Company-Overview.pdf" title="">ACCESS THE BROCHURE</a>`);
      }, 2000);
      document.querySelector('.heroContent').insertAdjacentHTML('afterbegin', `<div class="heroLeft">
        <h4>Canoe Intelligence</h4>
        ...
        <ul>
            ...
            <li><strong>Data extraction.</strong> Auto-extract, map, and normalize key data. Manage capital calls, distributions, fund metrics, and more.</li>
    ...
    <div class="heroRight">
      ...
      <div class="tab_form brochure_formOuter">
      </div>
      <ul>
        <li><strong>Document retrieval.</strong>  Auto-retrieve alts docs from inboxes & 500+ GP portals. Track in one place. Get alerts for late & missing docs.</li>
        <li><strong>Data extraction.</strong> Auto-extract, map, and normalize key data. Manage capital calls, distributions, fund metrics, and more.</li>
    ...
    `);
      ...
      document.querySelector('.heroContent').insertAdjacentHTML('afterbegin', `<div class="heroLeft">
        ...
        <h1>Optimize strategy with <br> deep <br> GP insights</h1>
        <ul>
    ...
    window.addEventListener('click', function (e) {
      function showDemoForm() {
        ...
        if (demoPopup) demoPopup.classList.add('active');
        if (brochurePopup) brochurePopup.classList.remove('active');
        localStorage.setItem('spz_3003_form', 'demo');
      }
      function showBrochureForm() {
        ...
        const demoPopup = document.querySelector('.demo_formOuter');
        const brochurePopup = document.querySelector('.brochure_formOuter');
        if (demoPopup) demoPopup.classList.remove('active');
        if (brochurePopup) brochurePopup.classList.add('active');
        localStorage.setItem('spz_3003_form', 'brochure');
      }
    ...
    setTimeout(() => {
      if (document.querySelector('.heroRight .load-wrapper')) {
        if (localStorage.getItem('spz_3003_form') === 'brochure') {
          console.log('Brochure form active');
          document.querySelectorAll('.form_wrapper .form_navBrochure').forEach(el => el.classList.add('active'));
          document.querySelectorAll('.form_wrapper .form_navDemo').forEach(el => el.classList.remove('active'));
          document.querySelector('.brochure_formOuter').classList.add('active');
          document.querySelector('.demo_formOuter').classList.remove('active');
        } else if (localStorage.getItem('spz_3003_form') === 'demo') {
          console.log('Demo form active');
          document.querySelectorAll('.form_wrapper .form_navDemo').forEach(el => el.classList.add('active'));
          document.querySelectorAll('.form_wrapper .form_navBrochure').forEach(el => el.classList.remove('active'));
          document.querySelector('.brochure_formOuter').classList.remove('active');
          document.querySelector('.demo_formOuter').classList.add('active');
        } else {
          console.log('Defaulting to demo form');
          document.querySelector('.demo_formOuter').classList.add('active');
        }
        document.querySelector('.heroRight .load-wrapper').remove();
      }
    }, 2000);
    document.querySelector('.hero_section').insertAdjacentHTML('afterend', `<div class="hero_sectionOuter"></div>`);
    document.querySelector('.hero_sectionOuter').insertAdjacentElement('afterbegin', document.querySelector('.hero_section'));
  }
  setTimeout(function () {
    clearInterval(bodyInterval3001);
  }, 5000)
