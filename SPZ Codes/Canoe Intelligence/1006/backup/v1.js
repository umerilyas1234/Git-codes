function spz1006() {
  if (!document.querySelector('body').classList.contains('spz_1006_v1')) {
    ...
    function formModification() {
      ...
      document.querySelector('.hero_section .gform-footer.gform_footer button')?.classList.add('spz_1006_v1_v1');
    }
            ...
            return `
              ...
              return `
                  ...
                  ``
                      ...
                      <div class="next-question spz_1006_v1_v1">Next</div>
                    </div>
                    `
          }).join('')
          }
    ...
    setTimeout(() => {
      ...
      ${ /*This html goes is inserted before the form */''}
        <div class="spz-company-logo"><img src="//res.cloudinary.com/spiralyze/image/upload/v1752679650/canoeintelligence/1006/logo.svg" alt="logo"/></div>
      `,
        customHTMLAfter: `
      ...
      const template_formUniqueSelector = "#row2.hero_section"

      const template_formLoaded = setInterval(() => {
        if (document.querySelector(template_formUniqueSelector) && document.querySelectorAll(`${template_formUniqueSelector} input`).length > 0) {
          clearInterval(template_formLoaded);
      ...
      ************************/
      ...
      function addForm(formData, formSelector) {
        ...
        <div class="spz-form-outer">
          <div class="spz-form-wrap hero_section spz_form">
      ...
      // setTimeout(() => {
      //   const fieldsToHide = ['.spz_form #input_3_5', '.spz_form #input_3_23'];
      //   fieldsToHide.forEach(selector => {
      //     document.querySelector(selector).closest('div').closest('div').classList.add('hide');
      //   });
      //   document.querySelectorAll('.spz_form .gfield input').forEach(input => {
      //     input.addEventListener('keyup', function () {
      //       const firstName = document.querySelector('.spz_form #input_3_15').value;
      //       const lastName = document.querySelector('.spz_form #input_3_16').value;
      //       const email = document.querySelector('.spz_form #input_3_2').value;
      //       const name = document.querySelector('.spz_form #input_3_4').value;

      //       // Check if all required fields are filled and email is valid
      //       if (firstName !== '' && lastName !== '' && email !== '' && name !== '') {
      //         console.log('All required fields are filled and email is valid.');
      //         // Select the elements to unhide
      //         const fieldsToShow = ['.spz_form #input_3_5', '.spz_form #input_3_23'];

      //         fieldsToShow.forEach(selector => {
      //           const fieldWrap = document.querySelector(selector).closest('div').closest('div');
