...
function load4002Test() {
  ...
  document.querySelector(".form-card .form-note a").innerHTML =
    "Privacy policy";
  setTimeout(() => {
    document.querySelector(
      '#event_hero .hs-form .input .hs-input[name="vwo_test_name__c"]'
    ).value = "#4004 | ShipBob | Quote LP | Below the Fold";
    document.querySelector(
      '#event_hero .hs-form .input .hs-input[name="vwo_test_treatment__c"]'
    ).value = "Variation 1";
  }, 5000);
  if (!sessionStorage.getItem("codeHasRun")) {
    document
      .querySelector(
        ".spz_t4002 .span12 .row-number-3 .hs_cos_wrapper_type_module"
      )
      .insertAdjacentHTML(
        "beforeEnd",
        `<div class="lock-herowrap"><div class="lock-hero"><p>Learn more</p><img src="//res.cloudinary.com/spiralyze/image/upload/v1701858348/shipbob/4002/chevrons-down.svg" alt="chevrons-down"></div></div>`
      );
  ...
  $(".lock-herowrap .lock-hero").on("click", function () {
    ...
    scrollToSection(".spz_t4002 .row-number-5");
    // refresh the slider
...
function loadTestCode4004() {
  if (!document.querySelector("body").classList.contains("spz_4004")) {
    ...
    console.log("111111");
    document.querySelector(
      ".spz_t4002 .section-stats-container ul li:first-child .stat-copy"
    ).textContent = "Of orders ship on time";
    document
      .querySelector(".sb-hero-section .sb-hero-section-form-container")
      .setAttribute("id", "event_hero");
    ...
    const selectInterval = setInterval(function () {
      if (
        document.querySelectorAll(".spz_t4002 .hs-form-field select").length > 1
      ) {
        clearInterval(selectInterval);
        for (
          let i = 0;
          i <
          document.querySelectorAll(".spz_t4002 .hs-form-field select").length;
          i++
        ) {
          document
            .querySelectorAll(".spz_t4002 .hs-form-field select")
